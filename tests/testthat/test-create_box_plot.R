...
test_that("create_box_plot() error if df parameter is NULL",{
  expect_error(RplotterPkg::create_box_plot())
})

test_that("create_box_plot() error if aes_y parameter is NULL",{
  expect_error(RplotterPkg::create_box_plot(
    df = RplotterPkg::organdata,
    aes_x = "country"
  ))
